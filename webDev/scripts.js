...
function showItemsInCart() {
    ...
    let total = document.querySelector(".totalPriceToPay");
    let table = document.querySelector(".productsSum");
    if (items && products) {
        ...
        Object.values(items).map(item => {
            ...
            cell1.innerHTML = `${item.name}<input type="hidden" name="names[]" value="${item.name}" />`;
            cell2.innerHTML = `$${item.price}<input type="hidden" name="prices[]" value="$${item.price}" />`;
            cell3.innerHTML = `${item.numOfItems}<input type="hidden" name="qty[]" value="${item.numOfItems}" />`;
            cell4.innerHTML = `$${item.price * item.numOfItems}<input type="hidden" name="totalItem[]" value="$${item.price * item.numOfItems}" />`;
            });
        total.innerHTML = `
                <h4>Total Price: $${totalCost}<input type="hidden" name="totalCost" value="$${totalCost}" /></h4>
        `;
    }
}
...
function reloadIfAddedToCart() { // To make sure modal will display newly added items.
